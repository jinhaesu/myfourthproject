          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
            {data.account_roles.map((a: any) => (
              <div key={a.label} className="flex items-center gap-2 text-xs">
                <span className="flex-1 truncate text-ink-800 dark:text-ink-100">{a.label}
                ...
                <select value={a.role}
                  ...
                  className="px-1.5 py-0.5 text-2xs rounded border border-ink-200 dark:border-ink-800 focus:outline-none">
      ...
      {query.isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="panel p-3">
              <div className="text-2xs text-ink-500 dark:text-ink-400">내부이체 건수</div>
              <div className="text-lg font-bold text-ink-900 dark:text-ink-50">{data?.transfer_count?.toLocaleString()}건</div>
            </div>
            <div className="panel p-3">
              <div className="text-2xs text-ink-500 dark:text-ink-400">총 이동 금액</div>
              <div className="text-lg font-bold text-ink-900 dark:text-ink-50">{formatCurrency(data?.total_amount || 0, false)}</div>
            </div>
            <div className="panel p-3">
              <div className="text-2xs text-ink-500 dark:text-ink-400">관련 계좌</div>
              <div className="text-lg font-bold text-ink-900 dark:text-ink-50">{accounts.length}개</div>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2">
                  <div className="text-center">
                    <div className="text-2xs text-ink-500 dark:text-ink-400">시작 잔액</div>
                    <div className="text-sm font-bold font-mono text-ink-700 dark:text-ink-300">{formatCurrency(data.period_balance.start_balance, false)}</div>
                  </div>
                  <ArrowRightIcon className="h-4 w-4 text-ink-300 dark:text-ink-600" />
                  <div className="text-center">
                    <div className="text-2xs text-ink-500 dark:text-ink-400">마감 잔액</div>
          ...
          {cf && (cf.reservoir_to_operating > 0 || cf.savings_move > 0) && (
            <div className="panel p-3">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                ...
                <div className="bg-canvas-50 dark:bg-ink-950 rounded-md p-2">
                  <div className="text-2xs text-ink-500 dark:text-ink-400">기타 이동</div>
                  <div className="text-sm font-bold text-ink-900 dark:text-ink-50 mt-0.5">{formatCurrency(cf.other_move, false)}</div>
                </div>
              </div>
            </div>
          )}
          ...
          {/* 이체 내역 */}
          <div className="panel overflow-hidden">
            <div className="px-3 py-2 border-b border-ink-200 dark:border-ink-800 text-2xs font-semibold text-ink-500 dark:text-ink-400 uppercase flex items-center justify-between">
            ...
            {transfers.length === 0 ? (
              ...
              <div className="divide-y divide-ink-50 dark:divide-ink-800 max-h-[560px] overflow-y-auto">
