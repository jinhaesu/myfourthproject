...
const KIND_TABS: Array<{ key: VoucherKind; label: string; tone: string; icon: any }> = [
  { key: 'sales_tax_invoice', label: '매출 세금계산서', tone: 'border-emerald-500 text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950', icon: ReceiptPercentIcon },
  { key: 'purchase_tax_invoice', label: '매입 세금계산서', tone: 'border-rose-500 text-rose-700 dark:text-rose-300 bg-rose-50 dark:bg-rose-950', icon: ReceiptPercentIcon },
  { key: 'sales_invoice', label: '매출 계산서(영세)', tone: 'border-emerald-400 dark:border-emerald-600 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950', icon: DocumentTextIcon },
  { key: 'purchase_invoice', label: '매입 계산서(영세)', tone: 'border-rose-400 dark:border-rose-600 text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950', icon: DocumentTextIcon },
  { key: 'card', label: '카드 매입', tone: 'border-blue-500 text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950', icon: CreditCardIcon },
  { key: 'cash_receipt', label: '현금영수증', tone: 'border-amber-500 text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950', icon: BanknotesIcon },
]
    ...
    <div className="space-y-3 max-w-7xl">
      ...
      <div className="flex items-end justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {lastVoucherNo && (
            <span className="text-2xs text-emerald-700 dark:text-emerald-300 font-semibold inline-flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <div className="panel p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <input
                ...
                placeholder="0"
                className="mt-0.5 w-full px-2 py-1.5 text-sm font-mono text-right rounded-md border border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950 focus:border-emerald-500 focus:outline-none font-semibold"
              />
            </div>
          </div>
          ...
          {duplicates && duplicates.length > 0 && (
            <div className="mt-2 rounded-md border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950 p-3 space-y-2">
              <div className="text-xs font-semibold text-amber-900 dark:text-amber-100 flex items-center gap-1">
                ⚠️ 비슷한 거래 {duplicates.length}건 이미 존재
              </div>
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {duplicates.map((d, i) => (
                  <div key={i} className="text-2xs text-amber-800 dark:text-amber-200 flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 pt-1 border-t border-amber-200 dark:border-amber-800">
                ...
                <button
                  ...
                  className="text-2xs font-semibold text-rose-700 dark:text-rose-300 underline"
                >
                  중복이어도 강제 저장
                </button>
              </div>
          ...
          {!supplyNum ? (
            ...
            <div className="space-y-3">
              ...
              <div className="text-2xs text-ink-500 dark:text-ink-400 px-1">
                ...
                {previewLines.debits.reduce((a, b) => a + Number(b.amount), 0) ===
                 previewLines.credits.reduce((a, b) => a + Number(b.amount), 0) ? (
                  <span className="text-emerald-600 dark:text-emerald-400 font-semibold">✓ 일치</span>
                ) : (
                  <span className="text-rose-600 dark:text-rose-400 font-semibold">✗ 불일치</span>
                )}
              </div>
            </div>
          )}
