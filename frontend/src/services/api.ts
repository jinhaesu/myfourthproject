...
// 환경변수 우선, 없으면 Cloud Run 서울 URL을 명시적 fallback (Vercel 환경변수 누락 방어)
// 로컬 개발 시 vite proxy 사용하려면 VITE_API_URL=/api/v1 설정
// 정본 백엔드 = Cloud Run 서울 (한국에서 ~0.1s, Railway 싱가포르는 ~0.33s)
const API_BASE_URL =
...
export const autoVoucherApi = {
  ...
  confirmBatch: (candidate_ids: number[], user_id: number = 1, background: boolean = false) =>
    api.post('/auto-voucher/confirm-batch', { candidate_ids, user_id, background }, { timeout: 300_000 }),
